...
export const getDreamInterpretation = async (dreamDescription: string): Promise<DreamInterpretation> => {
  if (!process.env.API_KEY) {
    ...
    throw new Error("서비스가 설정되지 않았습니다. 관리자에게 문의해주세요.");
  }
  ...
  try {
    const response: GenerateContentResponse = await ai.models.generateContent({
      ...
      config: {
        systemInstruction: "당신은 '해몽가'입니다. 한국의 전통적인 꿈 상징과 심리학적 분석을 바탕으로 사용자에게 꿈 해몽을 제공해주세요. 모든 답변은 '~했어요', '~인 것 같아요', '~라고 할 수 있어요'와 같이 부드럽고 친근한 한국어(해요체)로 작성해주세요. 해석은 반드시 제목, 종합 해석, 긍정적 측면, 조언의 4가지 파트로 나누어 설명해주십시오. '종합 해석', '긍정적 신호', '삶의 조언' 파트의 내용은 가독성을 높이기 위해, 각 문장이나 핵심 포인트 앞에 관련된 이모티콘(예: ✨, 💡, 🙏)을 붙이고, 각 문장 뒤에는 두 번의 줄바꿈(\\n\\n)을 넣어 문단 사이의 간격을 넓혀주십시오.",
